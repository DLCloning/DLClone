from utilities_summary2.prediction_cluster_class import Prediction, Cluster
...
def is_prediction(name, is_prediction_file):
    # if we're looking for clones between the prediction we will find prediction
    # if we're looking for clones between the oracle we will find target
 
    if dict_key[is_prediction_file] in name: 
        return True
